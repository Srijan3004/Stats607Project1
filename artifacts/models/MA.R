# ==============================================================================
# Script Name: MA.R
# Description: Implements Moving Average (MA) trend estimation (30-year window / 
#              360-month filter) and era-wise seasonality decomposition for 
#              key West Bengal stations (Alipur, Malda, Midnapore).
# Dependencies: zoo, dplyr, ggplot2, tidyr
# Output Files: artifacts/plots/ma_trend_*.png
#               artifacts/plots/ma_seasonality_*.png
# ==============================================================================

# Ensure environment and packages are available
...
# Imputing missing years using previous year data
...
year_station1 = rep(1901:2020, each = 12)
month = rep(1:12, 120)
...
dat1 = na.omit(df1 %>% filter(year_station1 %in% c(1901:1930)))
dat2 = na.omit(df1 %>% filter(year_station1 %in% c(1931:1960)))
dat3 = na.omit(df1 %>% filter(year_station1 %in% c(1961:1990)))
dat4 = na.omit(df1 %>% filter(year_station1 %in% c(1991:2020)))
...
# Plot trend and seasonality for ALIPUR
p_trend_alipur <- df2 %>% 
  ggplot() +
  aes(x = year_station1, y = ma12) +
  geom_line(color = "blue", linewidth = 1.2) +
  scale_x_continuous(breaks = seq(1901, 2020, by = 10), limits = c(1901, 2020)) +
...
p_season_alipur <- season %>% 
  ...
  labs(x = "Months", y = "Seasonality", title = "MA Seasonality Estimate for ALIPUR", color = "Time Period") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, angle = 45, hjust = 1),
...
ggsave("results/moving_average/ma_trend_alipur.png", p_trend_alipur, width = 8, height = 5)
ggsave("results/moving_average/ma_seasonality_alipur.png", p_season_alipur, width = 8, height = 5)
...
year_station2 = rep(1901:2020, each = 12)
month = rep(1:12, 120)
df2 = cbind.data.frame(year_station2, month, station2_rain)

# 30-year moving average trend estimation
...
dat21 = na.omit(df2 %>% filter(year_station2 %in% c(1901:1930)))
dat22 = na.omit(df2 %>% filter(year_station2 %in% c(1931:1960)))
dat23 = na.omit(df2 %>% filter(year_station2 %in% c(1961:1990)))
dat24 = na.omit(df2 %>% filter(year_station2 %in% c(1991:2020)))
...
season2 = cbind.data.frame(
  ...
  time2 = seasonality2_2,
  time3 = seasonality3_2, 
  time4 = seasonality4_2
)

# Plot trend and seasonality for MALDA
p_trend_malda <- df2_2 %>% 
  ggplot() +
  aes(x = year_station2, y = ma) +
  geom_line(color = "blue", linewidth = 1.2) +
  scale_x_continuous(breaks = seq(1901, 2020, by = 10), limits = c(1901, 2020)) +
  labs(x = "Year (Monthwise)", y = "Trend", title = "MA Trend Estimate for MALDA") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, angle = 45, hjust = 1),
...
p_season_malda <- season2 %>% 
  ggplot() +
  geom_line(aes(x = 1:12, y = time1, colour = "1901-1930")) +
  geom_line(aes(x = 1:12, y = time2, colour = "1931-1960")) +
  geom_line(aes(x = 1:12, y = time3, colour = "1961-1990")) +
  geom_line(aes(x = 1:12, y = time4, colour = "1991-2020")) +
  labs(x = "Months", y = "Seasonality", title = "MA Seasonality Estimate for MALDA", color = "Time Period") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, angle = 45, hjust = 1),
...
ggsave("results/moving_average/ma_trend_malda.png", p_trend_malda, width = 8, height = 5)
ggsave("results/moving_average/ma_seasonality_malda.png", p_season_malda, width = 8, height = 5)
...
new_dat_station3 <- dat_station3 %>% fill(`month.name`, .direction = "down")
dat_station3 = new_dat_station3

station3_rain = na.locf(as.numeric(t(dat_station3[, -1])))
year_station3 = rep(1901:2020, each = 12)
month = rep(1:12, 120)
...
dat31 = na.omit(df3 %>% filter(year_station3 %in% c(1901:1930)))
dat32 = na.omit(df3 %>% filter(year_station3 %in% c(1931:1960)))
dat33 = na.omit(df3 %>% filter(year_station3 %in% c(1961:1990)))
dat34 = na.omit(df3 %>% filter(year_station3 %in% c(1991:2020)))
...
season3 = cbind.data.frame(
  ...
  time2 = seasonality2_3,
  time3 = seasonality3_3, 
  time4 = seasonality4_3
)

# Plot trend and seasonality for MIDNAPORE
p_trend_midnapore <- df3_3 %>% 
  ggplot() +
  aes(x = year_station3, y = ma) +
  geom_line(color = "blue", linewidth = 1.2) +
  scale_x_continuous(breaks = seq(1901, 2020, by = 10), limits = c(1901, 2020)) +
  labs(x = "Year (Monthwise)", y = "Trend", title = "MA Trend Estimate for MIDNAPORE") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, angle = 45, hjust = 1),
...
p_season_midnapore <- season3 %>% 
  ggplot() +
  geom_line(aes(x = 1:12, y = time1, colour = "1901-1930")) +
  geom_line(aes(x = 1:12, y = time2, colour = "1931-1960")) +
  geom_line(aes(x = 1:12, y = time3, colour = "1961-1990")) +
  geom_line(aes(x = 1:12, y = time4, colour = "1991-2020")) +
  labs(x = "Months", y = "Seasonality", title = "MA Seasonality Estimate for MIDNAPORE", color = "Time Period") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, angle = 45, hjust = 1),
...
ggsave("results/moving_average/ma_trend_midnapore.png", p_trend_midnapore, width = 8, height = 5)
ggsave("results/moving_average/ma_seasonality_midnapore.png", p_season_midnapore, width = 8, height = 5)

MA_trend_midnapore = df3_3$ma
MA_season_midnapore = season3
MA_pred_midnpore = df3_3$ma + df3_3$sn

message("Moving Average calculations and plot generation completed for Alipur, Malda, and Midnapore.")