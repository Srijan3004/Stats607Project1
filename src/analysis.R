...
if (!dir.exists("results/model_plots")) {
  dir.create("results/model_plots", recursive = TRUE)
}

# ==============================================================================
# Load Caches & Hyperparameters (Add missing dependencies)
# ==============================================================================
if (file.exists("artifacts/models/FF1_cache.RData")) {
  load("artifacts/models/FF1_cache.RData")
}
if (file.exists("artifacts/models/FF1_cache_ext.RData")) {
  load("artifacts/models/FF1_cache_ext.RData")
}
if (file.exists("artifacts/models/lambda_seq.RData")) {
  load("artifacts/models/lambda_seq.RData")
}
...
lambda_seq <- c(0.01,  0.05,  0.10,  0.50,  1.00,  5.00, 10.00, 20.00)
# ======================================================================================
...
FF1_cache <- readRDS("artifacts/hyperparameter_mse/FF1_cache.Rds")
FF1_cache_ext <- readRDS("artifacts/hyperparameter_mse/FF1_cache_ext.Rds")
# ==============================================================================
# Environment Verification & Diagnostics
# ==============================================================================
cat("\n--- Running Environment Diagnostics ---\n")
cat("Total Stations:", nStn, "\n")
cat("Valid mse_h_ts entries:", sum(!is.na(mse_h_ts)), "/", nStn, "\n")
cat("Exists FF1_cache:", exists("FF1_cache"), "\n")
cat("Exists FF1_cache_ext:", exists("FF1_cache_ext"), "\n")
cat("Exists lambda_seq:", exists("lambda_seq"), "\n")

# Dry run test on Station 1
test_run <- try(trend_ss_estimate_smoothing(1, 8, mse_h_ts[1], 9), silent = TRUE)
if (inherits(test_run, "try-error")) {
  cat("WARNING: Diagnostic test failed for Station 1:\n")
  cat(attr(test_run, "condition")$message, "\n")
} else {
  cat("Diagnostic test successful for Station 1.\n")
}
cat("----------------------------------------\n\n")
...
valid_plots_count_1 <- 0

for (i in seq_len(nStn)) {
  
  if (is.na(mse_h_ts[i])) {
    message(sprintf("[Standard Boundary] Station %d skipped: mse_h_ts is NA", i))
    next
  }
  
  if (is.na(sum(mse_arr[i, , ]))) {
    message(sprintf("[Standard Boundary] Station %d skipped: NA detected in mse_arr", i))
    next
  }
  
  smooth_result <- try(trend_ss_estimate_smoothing(i, 8, mse_h_ts[i], 9), silent = TRUE)
  if (inherits(smooth_result, "try-error")) {
    message(sprintf("[Standard Boundary] Station %d skipped: trend_ss_estimate_smoothing failed (%s)", 
                    i, attr(smooth_result, "condition")$message))
    next
  }
  
  # If checks pass, execute plotting
  ...
  print(p1)
  print(p2)
  valid_plots_count_1 <- valid_plots_count_1 + 1
}

dev.off()
cat(sprintf("Standard Boundary PDF complete. Plotted %d / %d stations.\n", valid_plots_count_1, nStn))
...
valid_plots_count_2 <- 0

for (i in seq_len(nStn)) {
  
  if (is.na(mse_h_ts[i])) {
    message(sprintf("[Reflecting Boundary] Station %d skipped: mse_h_ts is NA", i))
    next
  }
  
  if (is.na(sum(mse_arr_ext[i, , ]))) {
    message(sprintf("[Reflecting Boundary] Station %d skipped: NA detected in mse_arr_ext", i))
    next
  }
  
  smooth_result <- try(trend_ss_estimate_smoothing_ext(i, 8, mse_h_ts[i], 9), silent = TRUE)
  if (inherits(smooth_result, "try-error")) {
    message(sprintf("[Reflecting Boundary] Station %d skipped: trend_ss_estimate_smoothing_ext failed (%s)", 
                    i, attr(smooth_result, "condition")$message))
    next
  }
  ...
  print(p1)
  print(p2)
  valid_plots_count_2 <- valid_plots_count_2 + 1
}

dev.off()
cat(sprintf("Reflecting Boundary PDF complete. Plotted %d / %d stations.\n", valid_plots_count_2, nStn))